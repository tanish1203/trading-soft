...
const ADMIN_PASSWORD = process.env.ADMIN_PASSWORD || "incorect"; // move to env on Render

// --- Next.js setup (serves the client app) ---
const dev = process.env.NODE_ENV !== "production";
// Tell Next that the app lives in ../client
...
nextApp.prepare().then(() => {
  ...
  // healthcheck stays fast
  app.get("/health", (_req, res) => res.json({ ok: true }));

  // Create HTTP server + socket.io on the SAME port
  ...
  */
  ...
  // Let Next handle everything else (/, /_next/*, etc.)
  app.all("*", (req, res) => handle(req, res));

  httpServer.listen(PORT, () => {
    console.log(`🚀 Server + Next running on :${PORT}`);
  });
});
