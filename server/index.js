...
game = {
  markets: Map<symbol, {
    ...
    clickSizeDefault: number,  // not used by clients anymore, here only for backward compat
    userStats: Map<userId, { buyVol:number, buyNotional:number, sellVol:number, sellNotional:number }>,
...
function onTradeHook(code, symbol, t) {
  ...
  const mk = g.markets.get(symbol); if (!mk) return;
  ...
  const upd = (uid, side, qty, px) => {
    if (!mk.userStats.has(uid)) mk.userStats.set(uid, { buyVol:0,buyNotional:0,sellVol:0,sellNotional:0 });
    const s = mk.userStats.get(uid);
    if (side === "buy") { s.buyVol += qty; s.buyNotional += qty*px; }
    else { s.sellVol += qty; s.sellNotional += qty*px; }
  };
...
function sendPersonalBundle(socket, code) {
  ...
  // personal books + positions and per-market user summary + implied total
  let total = 0;
  for (const [symbol, mk] of g.markets.entries()) {
    ...
    const st = mk.userStats.get(socket.id) || { buyVol:0,buyNotional:0,sellVol:0,sellNotional:0 };
    ...
    socket.emit("user_summary", {
      ...
      position: pos.qty || 0,
      avgBuy, avgSell,
      buyVol: st.buyVol, sellVol: st.sellVol,
    });
...
io.on("connection", (socket) => {
  ...
  socket.on("admin_create_game", ({ code, adminPassword, markets }) => {
    if (adminPassword !== ADMIN_PASSWORD) return socket.emit("admin_ack", { ok:false, error:"Bad password" });
    if (!/^\d{4}$/.test(String(code||""))) return socket.emit("admin_ack", { ok:false, error:"Code must be 4 digits" });
  ...
  socket.on("admin_toggle_market", ({ symbol, open }) => {
    const g = games.get(socket.data.code); if (!g) return;
  ...
  socket.on("admin_settle_all", ({ priceMap }) => {
    ...
    for (const [sym, v] of Object.entries(priceMap || {})) {
      const mk = g.markets.get(sym); if (!mk) continue;
      const px = OrderBook.snap(+v, mk.book.tickSize);
      mk.settlement = px; mk.open = false;
    }
    io.to(room(socket.data.code)).emit("markets_meta", packMarketsMeta(g));
    fanout(socket.data.code);
  });
  ...
  socket.on("place_order", ({ symbol, side, price, qty }) => {
    const g = games.get(socket.data.code); if (!g) return;
    const mk = g.markets.get(symbol); if (!mk || !mk.open) return;
  ...
  socket.on("click_trade", ({ symbol, side, price, maxQty }) => {
    const g = games.get(socket.data.code); if (!g) return;
    const mk = g.markets.get(symbol); if (!mk || !mk.open) return;
    mk.book.takeAtPrice({ userId: socket.id, side, price:+price, maxQty: Math.max(1, +maxQty || 1) });
    fanout(socket.data.code);
  });
